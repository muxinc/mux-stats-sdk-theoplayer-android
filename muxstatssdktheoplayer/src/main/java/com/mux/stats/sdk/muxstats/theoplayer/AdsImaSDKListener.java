...
public class AdsImaSDKListener implements AdErrorEvent.AdErrorListener, AdEvent.AdEventListener {
    private MuxStatsSDKTHEOplayer theoPlayerListener;
    ...
    public AdsImaSDKListener(MuxStatsSDKTHEOplayer listener) {
        theoPlayerListener = listener;
    }

    public void setTheoPlayerListener(MuxStatsSDKTHEOplayer listener) {
        theoPlayerListener = listener;
    }

    @Override
    public void onAdError(AdErrorEvent adErrorEvent) {
        if (theoPlayerListener != null) {
            PlaybackEvent event = new com.mux.stats.sdk.core.events.playback.AdErrorEvent(null);
            setupAdViewData(event, null);
            theoPlayerListener.dispatch(event);
        }
    }

    private void setupAdViewData(PlaybackEvent event, Ad ad) {
        ViewData viewData = new ViewData();
        if (theoPlayerListener.getCurrentPosition() == 0) {
            if (ad != null) {
